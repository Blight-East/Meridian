...
def generate_sales_message(lead_id):
    ...
    if not draft:
        # Fallback to template-based outreach
...
def _generate_with_claude(lead, checkout_url):
    ...
    prompt = f"""You are a sales intelligence agent for PayFlux, a payment risk observability company.
...
3. Position PayFlux as a simple way to see what changed earlier and keep a clearer record, not as a flashy sales pitch
4. Keep it concise. Stay under 150 words for the body.
5. Include the checkout URL naturally at the end
6. Never claim to be a person. This is from "PayFlux Intelligence".
7. Sound human, warm, and direct. Do not sound like marketing automation.
8. Prefer plain words over product jargon. If you mention PayFlux, explain it simply.
9. Avoid words like transformative, powerful, robust, strategic, pivotal, or cutting-edge.
10. Do not use em dashes, rule-of-three filler, or polished brochure language.
11. If the situation sounds active, say that plainly. If urgency is unclear, do not fake it.
12. Keep the note focused on one observation, one explanation, and one next step.
13. Never describe PayFlux as the payment processor, merchant account, gateway, or bank.
14. If the merchant needs a new processor, position PayFlux as the clarity and early-warning layer around that pressure, not as the replacement processor.
...
def _template_outreach(lead, checkout_url):
    ...
    return {
        "subject": f"Saw the processor pressure in {lead['industry']}",
        "body": (
            f"Hi {lead['merchant_name']},\n\n"
            f"We picked up signs of processor pressure affecting merchants in "
            f"{lead['industry']} using {lead['processor']}.\n\n"
            f"PayFlux is not the processor. It helps teams see risk earlier, "
            f"understand what changed, and keep a clearer record when pressure starts building.\n\n"
