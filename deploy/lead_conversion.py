...
def _build_outreach(merchant_name, processor, industry, checkout_url):
    return (
        f"Hi {merchant_name},\n\n"
        f"We've been tracking risk patterns across {processor} in the {industry} space "
        f"and noticed signals that match your processing profile.\n\n"
        f"PayFlux provides deterministic reserve projection — you'll see exactly where "
        f"your capital exposure is heading at T+30, T+60, and T+90, with signed reports "
        f"your board and processor can verify.\n\n"
...
def _build_cluster_outreach(merchant_name, processor, industry, cluster_topic,
    ...
    return (
        f"Hi {merchant_name},\n\n"
        f"We're reaching out because our systems have detected a {risk_level}-severity "
        f"distress cluster affecting {processor} merchants in {industry}.\n\n"
        f"The pattern — {cluster_topic} — is impacting multiple businesses in your vertical. "
        f"Merchants without forward-looking reserve intelligence are at elevated risk of "
        f"unexpected holds and cash flow disruption.\n\n"
        f"PayFlux gives you deterministic projections at T+30, T+60, and T+90 with signed "
        f"reports your board and processor can verify.\n\n"
        f"Activate your Pro subscription here:\n{checkout_url}\n\n"
        f"— PayFlux Team"
    )
