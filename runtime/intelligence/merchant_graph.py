...
def _init_relationships_table(conn):
    """Create persistent merchant_relationships table for graph edges."""
    conn.execute(text("""
    ...
    """))
    conn.execute(text("""
        CREATE INDEX IF NOT EXISTS idx_mr_merchant ON merchant_relationships(merchant_id)
    """))
    conn.execute(text("""
        CREATE INDEX IF NOT EXISTS idx_mr_neighbor ON merchant_relationships(neighbor_id)
    """))
    conn.commit()


def _store_relationship(conn, merchant_id, neighbor_id, relationship_type, confidence=0.6):
    """Persist a graph edge between two merchants."""
    if not merchant_id or not neighbor_id or merchant_id == neighbor_id:
        return
    try:
        conn.execute(text("""
            ...
            VALUES (:mid, :nid, :nid, :rtype, :conf)
            ON CONFLICT (merchant_id, neighbor_id, relationship_type) DO NOTHING
        """), {"mid": merchant_id, "nid": neighbor_id, "rtype": relationship_type, "conf": confidence})
    except Exception as e:
        logger.debug(f"Relationship insert error: {e}")


def _expand_2hop(conn, limit=MAX_2HOP_INSERTS):
    """
    2-hop graph propagation: find merchants reachable via 2 relationship hops
    from high-distress seeds, and create provisional records for unknown ones.
    """
    try:
    ...
    # Find 2-hop neighbors of high-distress merchants not yet expanded
    rows = conn.execute(text("""
        SELECT DISTINCT r2.neighbor_id, m2.canonical_name, m2.domain,
               m1.id as seed_id, m1.canonical_name as seed_name
        FROM merchant_relationships r1
        JOIN merchants m1 ON r1.merchant_id = m1.id
        JOIN merchant_relationships r2 ON r1.neighbor_id = r2.merchant_id
        JOIN merchants m2 ON r2.neighbor_id = m2.id
        WHERE m1.distress_score >= 10
          AND m2.status = 'provisional'
          AND m2.detected_from != 'graph_2hop'
        LIMIT :limit
    """), {"limit": limit}).fetchall()

    hop2_created = 0
    for neighbor_id, neighbor_name, neighbor_domain, seed_id, seed_name in rows:
        if hop2_created >= limit:
            break

        # Store the 2-hop relationship
        _store_relationship(conn, seed_id, neighbor_id, "graph_2hop", 0.4)
        hop2_created += 1

        save_event("merchant_graph_2hop", {
            "seed_merchant": seed_name,
            "neighbor_id": neighbor_id,
            "neighbor": neighbor_name,
        })

    logger.info(f"2-hop propagation: {hop2_created} edges created")
    return {"hop2_edges": hop2_created}
...
def run_merchant_graph_expansion(limit=GRAPH_BATCH_SIZE):
    ...
    Persists graph edges in merchant_relationships table.
    """
    ...
    result = {
        ...
        "hop2_edges": hop2_stats.get("hop2_edges", 0),
    }
    ...
    for cluster_id, cluster_topic, signal_ids_raw in clusters:
        ...
        for row in rows:
            ...
            for candidate in candidates:
                ...
                # Canonical duplicate check before creation
                if existing_id is None:
                    canonical_existing = check_and_log_canonical_match(conn, brand)
                    if canonical_existing:
                        existing_id = canonical_existing
                ...
                # Persist graph edge for each seed merchant
                for seed_mid in seed_ids:
                    _store_relationship(conn, seed_mid, merchant_id, "cluster_neighbor", 0.6)
                break
...
def _expand_from_co_mentions(conn, limit):
    ...
    for signal_id, content, source, signal_name, seed_merchant_id, seed_name, seed_distress in rows:
        ...
        for raw_candidate in _extract_related_brand_mentions(content or ""):
            ...
            # Canonical duplicate check before insertion
            canonical_existing = check_and_log_canonical_match(conn, cleaned)
            if canonical_existing:
                continue
            ...
            # Persist graph edge for co-mention relationship
            _store_relationship(conn, seed_merchant_id, merchant_id, "co_mention", 0.7)
            break
