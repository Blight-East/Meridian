...
def process_and_store_signal(source, content):
    ...
    with engine.connect() as conn:
        ...
        if similar:
            top_match = similar[0]
            similarity_score = top_match.get("score", top_match.get("similarity"))
        if similarity_score is not None and float(similarity_score) >= 0.92:
            matched_text = similar[0].get("text", similar[0].get("content"))
            if matched_text:
