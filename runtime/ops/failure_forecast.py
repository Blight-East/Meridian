    ...
    try:
        with engine.connect() as conn:
            ...
            for key, sigs_t in agg_t.items():
                ...
                horizon = 2
                predicted_vol = float(sigs_t + (trend_slope * 0.6 * horizon))
                
                if predicted_vol >= 15:
                    confidence = min(1.0, sigs_t / 20.0)
                    if confidence < 0.6:
                        continue
                    conn.execute(text("""
                        INSERT INTO processor_forecasts (processor, industry, predicted_volume, confidence)
                        VALUES (:p, :i, :v, :c)
                    """), {"p": p, "i": i, "v": predicted_vol, "c": confidence})
                    conn.commit()
                    ...
                    save_event("forecast_generated", {"processor": p, "industry": i, "predicted_volume": predicted_vol, "confidence": confidence})
                    logger.warning(f"Predictive forecast generated: {p} in {i} -> {predicted_vol} (conf: {confidence})")
                    forecasts += 1
